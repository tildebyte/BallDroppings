...
class Ball:
    def __init__(self, x=0, y=0, oldX=0, oldY=0, forceX=0, forceY=0, jitter=0):
        ...
        self.forceX = forceX
        self.forceY = forceY
        self.lastBounceTimes = [0 for x in range(16)]
        self.jitter = jitter
    ...
    def getForceRadius(self):
        # pythag to find dist
        return sqrt(self.forceX * self.forceX + self.forceY * self.forceY)
    ...
    def bounce(self):
        ...
        if self.bounceTimeDelta < self.tooMuchBouncingThreshold:
            ...
            if self.getForceRadius() >= 0.5 and self.getForceRadius() <= 10:
